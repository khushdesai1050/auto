...
export function Loans({ title, services }: LoanFormProps) {
  ...
  useEffect(() => {
    ...
    const servicesList = params.getAll("services");
    const finalServices = servicesList.length ? servicesList : services;

    setDynamicServices(finalServices);
    setFormData((p) => ({
      ...p,
      service: service && finalServices.includes(service) ? service : finalServices[0] || "",
    }));
